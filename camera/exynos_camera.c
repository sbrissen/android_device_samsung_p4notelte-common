...
struct exynos_camera_preset exynos_camera_presets_smdk4x12[] = {
	{
		...
		.params = {
			...
			.preview_frame_rate_values = "0",
			.preview_frame_rate = 0,
...
int exynos_camera_params_apply(struct exynos_camera *exynos_camera, int force)
{
	...
	if (preview_format_string != NULL) {
		if (strcmp(preview_format_string, "yuv420sp") == 0) {
		        ALOGE("SBRISSEN - preview_format = NV21");
	...
	if (picture_size_string != NULL) {
		...
		if (picture_width != 0 && picture_height != 0 && (picture_width != exynos_camera->picture_width || picture_height != exynos_camera->picture_height)) {
			...
			/*if (exynos_camera->camera_capture_format == V4L2_PIX_FMT_UYVY) {   //////////////////////////////////
	...
	if (picture_format_string != NULL) {
		if (strcmp(picture_format_string, "jpeg") == 0) {
		        ALOGE("SBRISSEN - Picture Format = JPEG\n");
	...
	if (recording_hint_string != NULL && strcmp(recording_hint_string, "true") == 0) {
		...
		if (exynos_camera->camera_fimc_is)
	        ...
	        ALOGE("%s - SBRISSEN - SENSOR_CAMERA", __func__); 
		camera_sensor_mode = SENSOR_CAMERA;

		if (exynos_camera->camera_fimc_is ){
		        ALOGE("%s - SBRISSEN - IS_MODE_PREVIEW_STILL", __func__); 
	...
	if (exynos_camera->camera_fimc_is && fimc_is_mode != exynos_camera->fimc_is_mode) {
		...
		ALOGE("%s - SBRISSEN - exynos_camera->camera_fimc_is ", __func__); 
	...
	if (antibanding_string != NULL) {
		if (strcmp(antibanding_string, "50hz") == 0)
...
/*int isx012_interleaved_decode(void *data, int size,
	...
	void *jpeg_data, int *jpeg_size, int *decoded, int *auto_focus_result)
{
	...
	unsigned char isx012_auto_focus_result;
...
//	switch (format) {
		/*case V4L2_PIX_FMT_UYVY:
			...
			rc = isx012_interleaved_decode(pointer, buffer_length, exynos_camera->capture_yuv_buffer, &yuv_length, width, height, exynos_camera->capture_jpeg_buffer, &jpeg_length, &decoded, &auto_focus_result);
			if (rc < 0) {
				ALOGE("%s: Unable to decode ISX012 interleaved", __func__);
				goto error;
			}
...
int exynos_camera_capture_start(struct exynos_camera *exynos_camera, int is_capture)
{
	...
	if (exynos_camera->camera_fimc_is) {
	        ALOGE("%s: exynos_camera->camera_fimc_is 1", __func__);
		rc = exynos_v4l2_s_ctrl(exynos_camera, 0, V4L2_CID_IS_S_FORMAT_SCENARIO, exynos_camera->fimc_is_mode);
		if (rc < 0) {
			ALOGE("%s: Unable to set FIMC-IS scenario", __func__);
			goto error;
	...
	if (exynos_camera->camera_fimc_is) {
		// Set MBUS width/height/format
	        ALOGE("SBRISSEN - V4L2_PIX_FMT_MODE_PREVIEW 1");
...
if(!is_capture) {
	ALOGE("SBRISSEN - V4L2_PIX_FMT_MODE_PREVIEW 2");
	...
	if (!exynos_camera->camera_fimc_is) {
		// Set MBUS width/height/format
	        ALOGE("SBRISSEN - V4L2_PIX_FMT_MODE_PREVIEW 3");
	...
	ALOGE("SBRISSEN - V4L2_PIX_FMT_MODE_CAPTURE 2");
	...
	if (!exynos_camera->camera_fimc_is) {
		// Set MBUS width/height/format
	        ALOGE("SBRISSEN - V4L2_PIX_FMT_MODE_CAPTURE 3");
	...
	ALOGE("SBRISSEN - mbus_height %i mbus_width %i height %i width %i\n", mbus_height, mbus_width, height, width);
...
complete:
        ALOGE("SBRISSEN -  Capture_start DONE!\n");
	return rc;
}
...
void exynos_camera_capture_stop(struct exynos_camera *exynos_camera)
{
	...
	/*if (exynos_camera->capture_format == V4L2_PIX_FMT_UYVY) {
...
list_continue:
	...
	if (!exynos_camera->capture_enabled) {
		...
		exynos_camera->capture_height = height;
		exynos_camera->capture_format = format;
                ALOGE("%s : capture enabled",  __func__);
...
complete:
        ALOGE("SBRISSEN - exynos_camera_preview_output_start -DONE\n");
	return rc;
}
...
//	ALOGD("%s()", __func__);
	...
	for (i = 0; i < buffers_count; i++) {
		...
		/*if (buffers->format == V4L2_PIX_FMT_UYVY){
		        ALOGE("SBRISSEN - exynos_camera_preview_callback - interleaved\n");
			goto buffers_continue;}*/
...
buffers_continue:
	...
	if (buffer_width != width || buffer_height != height || buffer_format != format) {
		if (!exynos_camera->preview_output_enabled) {
			...
			ALOGE("SBRISSEN - exynos_camera_preview_callback -if\n");
			...
			memcpy(&exynos_camera->preview_buffer, buffer, sizeof(struct exynos_camera_buffer));
                        ALOGE("SBRISSEN - exynos_camera_preview_callback -else\n");
			rc = exynos_camera_preview_output_start(exynos_camera);
			if (rc < 0) {
				ALOGE("%s: Unable to start preview", __func__);
				goto error;
	...
	struct exynos_camera_buffer *buffers, int buffers_count)
{
	...
	//if (!exynos_camera->picture_enabled && !exynos_camera->camera_fimc_is) {
		...
		/*if (exynos_camera->camera_capture_format == V4L2_PIX_FMT_UYVY && !exynos_camera->capture_hybrid) {
	...
	for (i = 0; i < buffers_count; i++) {
		if (buffers->format == V4L2_PIX_FMT_JPEG) {
		        ALOGE("SBRISSEN - exynos_camera_picture_callback - JPEG\n");
			if (buffers->width == width && buffers->height == height)
				jpeg_buffer = buffers;
...
/*	if (exynos_camera->camera_capture_format == V4L2_PIX_FMT_UYVY && jpeg_buffer == NULL) {
	        ALOGE("SBRISEN - exynos_camera_picture_callback - picture_listener->busy\n");
		exynos_camera->picture_listener->busy = 0;
		return 0;
	}*/
...
int exynos_camera_picture(struct exynos_camera *exynos_camera)
{
	...
	if (jpeg_data == NULL) {
	        ALOGE("SBRISSEN - exynos_camera_picture\n");
	...
	struct exynos_camera_buffer *buffers, int buffers_count)
{
	...
	for (i = 0; i < buffers_count; i++) {
		/*if (buffers->format == V4L2_PIX_FMT_JPEG){
		        ALOGE("SBRISSEN - xynos_camera_recording_callback V4L2_PIX_FMT_JPEG\n");
			goto buffers_continue;}

		if (buffers->format == V4L2_PIX_FMT_UYVY){
		        ALOGE("SBRISSEN - xynos_camera_recording_callback V4L2_PIX_FMT_UYVY\n");
			goto buffers_continue;}*/

		// Optimal buffer
		if (buffers->width == width && buffers->height == height) {
		        ALOGE("SBRISSEN - xynos_camera_recording_callback optimal buffer\n");
			buffer = buffers;
			break;
		}
...
//	ALOGD("%s()", __func__);
	...
	switch (auto_focus_status) {
	  case 0x08://CAMERA_AF_STATUS_IN_PROGRESS:
/*
...
*/
			break;
	  case 0x02://CAMERA_AF_STATUS_SUCCESS:
			...
			break;
	  case 0x01://CAMERA_AF_STATUS_FAIL:
		default:
			if (EXYNOS_CAMERA_MSG_ENABLED(CAMERA_MSG_FOCUS) && EXYNOS_CAMERA_CALLBACK_DEFINED(notify) && !exynos_camera->callback_lock)
...
{
	struct exynos_camera *exynos_camera;
	int auto_focus_status = 0x01; //CAMERA_AF_STATUS_FAIL;
	...
	while (exynos_camera->auto_focus_thread_enabled) {
		...
		if (rc < 0) {
			...
			auto_focus_status = 0x01;//CAMERA_AF_STATUS_FAIL;
		}
		...
		if (auto_focus_status == 0x08)//CAMERA_AF_STATUS_IN_PROGRESS)
		        usleep(10000);
		else
...
int exynos_camera_auto_focus_thread_start(struct exynos_camera *exynos_camera)
{
	...
	//auto_focus = AUTO_FOCUS_MAX;// | (exynos_camera->preview_width & 0xfff) << 20 | (exynos_camera->preview_height & 0xfff) << 8;
	...
	struct preview_stream_ops *w)
{
	...
	rc = w->set_usage(w, GRALLOC_USAGE_SW_WRITE_OFTEN | GRALLOC_USAGE_HWC_HWOVERLAY | GRALLOC_USAGE_HW_ION);
	if (rc) {
